#include <iostream>
#include <string>
#include <vector>
#include <cctype>

class RecursiveDescentParser {
private:
    std::string input;
    int position;
    char current_char;
    bool error;
    std::vector<std::string> steps;

public:
    RecursiveDescentParser(const std::string& str) 
        : input(str), position(0), error(false) {
        if (!input.empty()) {
            current_char = input[0];
        }
    }

    void nextChar() {
        position++;
        if (position < input.length()) {
            current_char = input[position];
        } else {
            current_char = '\0';
        }
    }

    void skipWhitespace() {
        while (current_char == ' ' || current_char == '\t' || current_char == '\n') {
            nextChar();
        }
    }

    bool match(char expected) {
        skipWhitespace();
        if (current_char == expected) {
            nextChar();
            return true;
        }
        return false;
    }

    void reportError(const std::string& message) {
        error = true;
        std::cout << "错误: " << message << " (位置 " << position << ")" << std::endl;
        std::cout << "输入: " << input << std::endl;
        std::cout << std::string(position, ' ') << "^" << std::endl;
    }

    void printStep(const std::string& production) {
        steps.push_back(production);
        std::cout << "使用产生式: " << production << std::endl;
    }

    // E -> T G
    void E() {
        printStep("E -> T G");
        T();
        if (!error) G();
    }

    // G -> + T G | - T G | ε
    void G() {
        skipWhitespace();
        if (current_char == '+') {
            printStep("G -> + T G");
            if (!match('+')) {
                reportError("期望 '+'");
                return;
            }
            T();
            if (!error) G();
        } else if (current_char == '-') {
            printStep("G -> - T G");
            if (!match('-')) {
                reportError("期望 '-'");
                return;
            }
            T();
            if (!error) G();
        } else {
            printStep("G -> ε");
            // ε 产生式，什么都不做
        }
    }

    // T -> F S
    void T() {
        printStep("T -> F S");
        F();
        if (!error) S();
    }

    // S -> * F S | / F S | ε
    void S() {
        skipWhitespace();
        if (current_char == '*') {
            printStep("S -> * F S");
            if (!match('*')) {
                reportError("期望 '*'");
                return;
            }
            F();
            if (!error) S();
        } else if (current_char == '/') {
            printStep("S -> / F S");
            if (!match('/')) {
                reportError("期望 '/'");
                return;
            }
            F();
            if (!error) S();
        } else {
            printStep("S -> ε");
            // ε 产生式，什么都不做
        }
    }

    // F -> ( E ) | i
    void F() {
        skipWhitespace();
        if (current_char == '(') {
            printStep("F -> ( E )");
            if (!match('(')) {
                reportError("期望 '('");
                return;
            }
            E();
            if (!error) {
                if (!match(')')) {
                    reportError("期望 ')'");
                }
            }
        } else if (current_char == 'i') {
            printStep("F -> i");
            if (!match('i')) {
                reportError("期望 'i'");
                return;
            }
        } else {
            reportError("期望 '(', 'i' 或标识符");
        }
    }

    void showFullDerivation() {
        std::cout << "\n=== 完整推导过程 ===" << std::endl;
        for (size_t i = 0; i < steps.size(); i++) {
            std::cout << "步骤 " << (i + 1) << ": " << steps[i] << std::endl;
        }
    }

    bool parse() {
        std::cout << "========================================" << std::endl;
        std::cout << "递归下降分析程序" << std::endl;
        std::cout << "编制人：方泽宇，2351610105，23508014" << std::endl;
        std::cout << "========================================" << std::endl;
        std::cout << "输入符号串: " << input << std::endl;
        std::cout << "========================================" << std::endl;
        
        steps.clear();
        error = false;
        
        std::cout << "开始语法分析..." << std::endl;
        
        E();
        
        if (!error) {
            skipWhitespace();
            if (current_char != '#' && current_char != '\0') {
                reportError("期望结束符 '#' 或字符串结束");
            } else if (current_char == '#') {
                match('#');
            }
        }
        
        showFullDerivation();
        
        std::cout << "\n========================================" << std::endl;
        if (!error) {
            std::cout << "✓ 分析成功: " << input << " 为合法符号串" << std::endl;
            std::cout << "总推导步骤: " << steps.size() << std::endl;
            return true;
        } else {
            std::cout << "✗ 分析失败: " << input << " 为非法符号串" << std::endl;
            return false;
        }
    }
};

int main() {
    std::cout << "语法分析器 - 递归下降分析法" << std::endl;
    std::cout << "文法规则:" << std::endl;
    std::cout << "E -> T G" << std::endl;
    std::cout << "G -> + T G | - T G | ε" << std::endl;
    std::cout << "T -> F S" << std::endl;
    std::cout << "S -> * F S | / F S | ε" << std::endl;
    std::cout << "F -> ( E ) | i" << std::endl;
    std::cout << std::endl;
    
    std::string input;
    std::cout << "请输入要分析的符号串 (以#结束): ";
    std::getline(std::cin, input);
    
    if (!input.empty() && input.back() != '#') {
        input += '#';
    }
    
    RecursiveDescentParser parser(input);
    parser.parse();
    
    return 0;
}